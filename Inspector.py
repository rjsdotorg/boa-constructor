...
from Preferences import IS, oiLineHeight, inspPageNames
...
class InspectorFrame(wx.Frame, Utils.FrameRestorerMixin):
    ...
    def __init__(self, parent):
        ...
        self.wxID_CANCEL = Utils.AddToolButtonBmpIS(self, self.toolBar,
                                                    'Images/Inspector/Cancel.png', _('Cancel the session'),
                                                    self.OnCancel)
        self.toolBar.AddSeparator()
...
class ParentTree(wx.TreeCtrl):
    ...
    def __init__(self, parent):
        wx.TreeCtrl.__init__(self, parent, wxID_PARENTTREE,
                             style=wx.TR_HAS_BUTTONS | wx.CLIP_CHILDREN | wx.SUNKEN_BORDER)
        self.cleanup()
...
class NameValue:
    ...
    def __init__(self, inspector, nameParent, valueParent, companion,
        ...
        if editor:
            ...
            self.propEditor = self.inspector.inspector.propertyRegistry.factory(name,
                                                                                valueParent, companion, rootCompanion,
                                                                                propWrapper, idx,
        ...
        self.value = wx.lib.stattext.GenStaticText(valueParent, -1, displayVal,
                                                   ...
                                                   wx.Size(inspector.getValueWidth(),
                                                           oiLineHeight - 3),
                                                   style=wx.ST_NO_AUTORESIZE)  # wxCLIP_CHILDREN |
        ...
        self.separatorN = wx.Window(nameParent, -1, wx.Point(0,
                                                             (idx + 1) * oiLineHeight),
                                    wx.Size(inspector.panelNames.GetSize().x, 1))  # ,
        ...
        self.separatorV = wx.Window(valueParent, -1, wx.Point(0,
                                                              (idx + 1) * oiLineHeight),
                                    wx.Size(inspector.getValueWidth(), 1))  # ,
        # style=wx.CLIP_CHILDREN)
    ...
    def OnExpand(self, event):
        if event.IsChecked():
            self.inspector.collapse(self)
        else:
            self.inspector.expand(self)
...
class InspectorConstrScrollWin(InspectorScrollWin):
    """ Specialised InspectorScrollWin that understands constructor parameters """
            ...
            self.nameValues.insert(len(self.nameValues),
                                   EventNameValue(self, self.panelNames,
                                                  ...
                                                  name, RTTI.PropertyWrapper(name, 'EventRoute',
                                                                             self.inspector.selCmp.evtGetter,
                                                                             self.inspector.selCmp.evtSetter),
